...
with open('data/Chile/20191227_dump/csvs/2019.csv', mode='r') as csv_file:
    ...
    for row in csv_reader:
        ...
        STATE = row[0].upper()
        STATE_ISO = None
        FIRE_NAME = row[5].upper()

        # Convert ha to acres
        SIZE =  float(row[23]) * 2.4710538 
        ...
        # Dates in locale datetime have format 18-oct-2002 14:51, 18-oct-2002 19:00
        START_DATE = datetime.datetime.strptime(row[24], '%d-%b-%Y %H:%M').date()  # look good
        try:
            END_DATE = datetime.datetime.strptime(row[25], '%d-%b-%Y %H:%M').date()  # look good
        except ValueError as e:  # this accounts for end_date blanks
            END_DATE = None

        # Cause can only be LIGHTNING, HUMAN, or UNKNOWN
        CAUSE = row[9].split(' ')[0] 
        if CAUSE in chile_causes:
            CAUSE = chile_causes[CAUSE][0]
        else:
            raise KeyError("CAUSE NOT FOUND")
