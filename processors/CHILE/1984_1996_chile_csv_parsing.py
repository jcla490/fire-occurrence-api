...
with open('data/Chile/20191227_dump/csvs/1985_1996.csv', mode='r') as csv_file:
    ...
    for row in csv_reader:
        ...
        CAUSE = row[7].split(' ')[0] 
        if CAUSE in ['NO IDENTIFICADA - Desconocida', 'OTRAS ACTIVIDADES - Otras', '', 'NO IDENTIFICADA: Desconocida']:
            CAUSE = 'UNKNOWN'
       ...
       # Convert ha to acres
        try:
            SIZE =  float(row[21]) * 2.4710538 
        except ValueError:
            SIZE = float(row[21].replace(',', '')) * 2.4710538 

        # Dates in locale datetime have format 18-oct-2002 14:51, 18-oct-2002 19:00 or 3/22/18 23:00
        try: 
            START_DATE = datetime.datetime.strptime(row[8], '%d-%b-%Y %H:%M').date() 
        except ValueError: 
            START_DATE = datetime.datetime.strptime(row[8], '%m/%d/%y %H:%M').date() 
        try:
            END_DATE = datetime.datetime.strptime(row[9], '%d-%b-%Y %H:%M').date() 
        except ValueError:  # this accounts for end_date blanks
            try: 
                END_DATE = datetime.datetime.strptime(row[9], '%m/%d/%y %H:%M').date() 
            except: 
                END_DATE = None
