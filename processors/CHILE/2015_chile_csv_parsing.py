...
with open('data/Chile/20191227_dump/csvs/2015.csv', mode='r') as csv_file:
    ...
    for row in csv_reader:
        ...
        try: 
            START_DATE = datetime.datetime.strptime(row[24], '%d-%b-%Y %H:%M').date() 
        except ValueError: 
            START_DATE = datetime.datetime.strptime(row[24], '%m/%d/%y %H:%M').date() 
        try:
            END_DATE = datetime.datetime.strptime(row[25], '%d-%b-%Y %H:%M').date() 
        except ValueError:  # this accounts for end_date blanks
            try: 
                END_DATE = datetime.datetime.strptime(row[25], '%m/%d/%y %H:%M').date() 
            except: 
                END_DATE = None
