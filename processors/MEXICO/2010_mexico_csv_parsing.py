# No start dates = 45 fires for 450.4 ha
...
acres = 0
fires = 0
with open('/Users/joshuaclark/Desktop/repos/fire-occurrence-api/data/Mexico/csvs/Serie_historica_anual_incendios_(2010).csv', mode='r') as csv_file:
    ...
    for row in csv_reader:
        ...
        try: 
            START_DATE = datetime.datetime.strptime(row[7], '%d/%m/%y').date()  
        except:
            START_DATE = datetime.datetime.strptime(row[7], '%d-%m-%Y').date()  
        try:    
            END_DATE = datetime.datetime.strptime(row[8], '%d/%m/%y').date() 
        except:
            try:
                END_DATE = datetime.datetime.strptime(row[8], '%d-%m-%Y').date()
            except:
        ...
        COUNTRY_ISO = 'MX'
        try:
            SIZE =  float(row[13]) * 2.4710538  #probably
        except ValueError:
            SIZE = float(row[13].replace(',', '')) * 2.4710538 
        CAUSE = row[4] # ye
