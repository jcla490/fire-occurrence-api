...
acres = 0
fires = 0
with open('/Users/joshuaclark/Desktop/repos/fire-occurrence-api/data/Mexico/csvs/Serie_historica_anual_incendios_(2012).csv', mode='r') as csv_file:
    ...
    for row in csv_reader:
        ...
        try: 
            START_DATE = datetime.datetime.strptime(row[8], '%d/%m/%y').date()  # look good
        except:
            START_DATE = datetime.datetime.strptime(row[8], '%d-%m-%Y').date()  # look good
        try:    
            END_DATE = datetime.datetime.strptime(row[9], '%d/%m/%y').date()  # okay
        except:
            END_DATE = datetime.datetime.strptime(row[9], '%d-%m-%Y').date()  # look good

        FIRE_NAME = row[5]  # just take whatever this says
        STATE = row[3]  #ok
        COUNTRY_ISO = 'MX'
        try:
            SIZE =  float(row[21]) * 2.4710538  #probably
        except ValueError:
